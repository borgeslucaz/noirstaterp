...
RegisterRawNuiCallback('chatResult', function(requestData, cb)
  ...
  local ok, data = pcall(json.decode, requestData.body or '')
  if not ok or type(data) ~= 'table' then
    cb({ body = 'invalid payload' })
    return
  end

  if not data.canceled then
    if type(data.message) ~= 'string' then
      cb({ body = 'invalid message' })
      return
    end

    local maxLength = tonumber(Config.MaxMessageLength) or 500
    if #data.message > maxLength then
      data.message = data.message:sub(1, maxLength)
    end

    if data.mode ~= nil and type(data.mode) ~= 'string' then
      data.mode = nil
    end
...
if RegisterKeyMapping and type(Config.ToggleChatKey) == 'string' and Config.ToggleChatKey ~= '' then
  RegisterKeyMapping('toggleChat', 'Toggle chat', 'keyboard', Config.ToggleChatKey)
end
