...
Config.InteractionHandler = 'ox_target'
...
-- XP por entrega é determinístico (Config.Routes[...].baseXP × nota). Ver shared/contract_config.lua
...
-- Fonte física temporária usada apenas para preservar os spawns e fluxos da V1.
-- O restante do resource consome exclusivamente Config.Routes, abaixo.
Config.RouteSource = {
...
Config.Routes = {}
local noirRouteNumber = 0
for _, source in ipairs(Config.RouteSource) do
  for sourceIndex, physical in ipairs(source.routes or {}) do
    noirRouteNumber = noirRouteNumber + 1
    local meta = noirRoutes[noirRouteNumber]
    Config.Routes[#Config.Routes + 1] = {
      id = ('route_%02d'):format(noirRouteNumber), title = source.header,
      routeLabel = physical.label, image = source.image, smallImage = source.small_image or source.image,
      tier = meta[1], estimatedMinutes = meta[2], baseXP = meta[3], reqLevel = meta[4], cargoType = meta[5],
      vehicles = physical.vehicle or {}, trailerModel = physical.trailerModel,
      pickupSpawns = physical.trailerSpawnAvaliableCoords, destination = physical.destination,
      physical = physical, sourceId = source.id, sourceIndex = sourceIndex,
    }
  end
end

function GetRoute(routeId)
  for _, route in ipairs(Config.Routes) do if route.id == routeId then return route end end
end

-- Adaptadores internos para os fluxos físicos preservados. Nenhum destes
-- identificadores é enviado ao cliente nem gravado no banco da V2.
function ResolveCatalogRoute(routeId, sourceIndex)
  local route = sourceIndex and nil or GetRoute(routeId)
  if sourceIndex then
    for _, candidate in ipairs(Config.Routes) do
      if candidate.sourceId == tonumber(routeId) and candidate.sourceIndex == tonumber(sourceIndex) then route = candidate break end
    end
  end
  if not route then return nil, nil end
  return { header = route.title, image = route.image, small_image = route.smallImage }, route.physical
end

function GetRoutes(routeId)
  local route = GetRoute(routeId)
  return route and route or nil
end
