...
--- @param result table Full result breakdown (grade, score, xp, )
...
function Open.AddMoney(source, amount, moneyType)
    if HasBgrzCore() then
        local ok, res = pcall(function()
            return exports.bgrz_core:AddMoney(source, moneyType or 'cash', amount, 'noir-truckjob')
        end)
...
function Open.RemoveMoney(source, amount, moneyType)
    if HasBgrzCore() then
        local ok, res = pcall(function()
            return exports.bgrz_core:RemoveMoney(source, moneyType or 'cash', amount, 'noir-truckjob')
        end)
        if ok then return res == true end
        return false
    end
