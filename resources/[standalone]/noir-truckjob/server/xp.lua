...
-- Config.Routes × nota, nunca de sorteio nem do client).
-- ============================================================

local function Persist(identifier, myData)
    ExecuteSqlAsync(
        'UPDATE noir_truckjob_players SET `level` = :level, `xp` = :xp WHERE `identifier` = :id',
        { level = myData.level, xp = myData.xp, id = identifier }
    )
end

--- Adds XP to a player, applying as many level-ups as the amount allows.
--- Mission availability is derived from level at read time, so
