...
local function maintainBlur()
    ...
    local focusDist = Photomode.focusDistance
    if focusDist <= 0.0 then
        focusDist = math.sqrt((Photomode.distance ^ 2) + ((Photomode.height - 0.5) ^ 2))
        Photomode.focusDistance = focusDist
    end
...
local function updateCamera()
    ...
    Photomode.focusDistance = math.sqrt((Photomode.distance ^ 2) + ((Photomode.height - 0.5) ^ 2))
...
local function initCameraFromPed()
    local ped = Photomode.ped or PlayerPedId()
    ...
    CreateThread(function()
        while Photomode.active do
            if Photomode.animating then
                ...
                -- The scripted camera and DOF settings persist. While idle, a
                -- slower refresh is enough to follow an externally moved ped.
                updateCamera()
                Wait(Config.PhotomodeIdleRefreshMs or 100)
            end
        end
    end)
end
