...
local function RouteAvailable(profile, route)
    local level = tonumber(profile.level) or 1
    if level < (tonumber(route.reqLevel) or 1) then
        return false, { key = 'lock_mission_level', args = { route.reqLevel }, err = 'err_mission_level', errArgs = { route.reqLevel } }
    end
    return true, nil
end
...
function Contracts.CheckEligibility(profile, identifier, offer, activeSession)
    ...
    local route = GetRoute(offer.routeId)
    if not route then
    ...
    for _, name in ipairs(route.vehicles or {}) do
...
function Contracts.Start(src, data)
    ...
    local affected = ExecuteSqlUpdate(
        "UPDATE noir_truckjob_offers SET status = 'in_progress', driver_identifier = ?, started_at = NOW() WHERE offer_id = ? AND rotation_id = ? AND status = 'available'",
    ...
    local inserted = ExecuteSqlUpdate(
        "INSERT INTO noir_truckjob_deliveries (session_id, identifier, rotation_id, offer_id, route_id, tier, status, started_at) VALUES (?, ?, ?, ?, ?, ?, 'in_progress', NOW())",
        { session.sessionId, identifier, rotationId, offerId, offer.routeId, offer.tier }
    )
...
function Contracts.Finish(src, sessionId, clientHealth)
    ...
    local result = {
        ...
        title = mission.header,
        routeLabel = route.label,
        cargoType = (GetRoute(session.routeId) or {}).cargoType,
    ...
    ExecuteSqlAsync(
        'UPDATE noir_truckjob_players SET `totalEarnings` = :earnings, `completedJobs` = :jobs, `globalCompleted` = :gc WHERE `identifier` = :id',
        {
    ...
    ExecuteSqlAsync(
        "UPDATE noir_truckjob_offers SET status = 'completed', finished_at = NOW(), result_reason = 'completed' WHERE offer_id = ? AND rotation_id = ?",
    ...
    if reward.damagePenalty > 0 then
        TriggerClientEvent('noir-truckjob:createNotification', src, L('you_charged', Peak.Utils.FormatNumber(reward.damagePenalty)))
    end
    TriggerClientEvent('noir-truckjob:jobResult', src, result)
    ...
    Log('%s concluiu %s: nota %s (%.1f) total $%d xp %d em %.1f min',
        session.identifier, session.offerId, evaluation.grade, evaluation.score, reward.total, reward.xp, evaluation.elapsedMinutes)
...
function Contracts.Fail(session, status, reason)
    ...
    ExecuteSqlAsync(
        "UPDATE noir_truckjob_deliveries SET status = ?, result_reason = ?, finished_at = NOW() WHERE session_id = ? AND status = 'in_progress'",
        { status, reason, session.sessionId }
    )
    ExecuteSqlAsync(
        "UPDATE noir_truckjob_offers SET status = ?, finished_at = NOW(), result_reason = ? WHERE offer_id = ? AND rotation_id = ? AND status = 'in_progress'",
    ...
    if status == 'failed' and profile and profile.identifier == session.identifier then
        ...
        ExecuteSqlAsync(
            'UPDATE noir_truckjob_players SET `failedJobs` = :fj, `globalFailed` = :gf WHERE `identifier` = :id',
    ...
    elseif status == 'failed' then
        ExecuteSqlAsync(
            'UPDATE noir_truckjob_players SET `failedJobs` = `failedJobs` + 1, `globalFailed` = `globalFailed` + 1 WHERE `identifier` = :id',
    ...
    if src and GetPlayerPed(src) ~= 0 then
        TriggerClientEvent('noir-truckjob:jobFailed', src, { status = status, reason = reason, sessionId = session.sessionId })
...
function Contracts.ResolveSuspended(src, identifier)
    ...
    TriggerClientEvent('noir-truckjob:createNotification', src, L('job_recovered_closed'))
end
...
function Contracts.OnResourceStop()
    for _, session in pairs(ActiveJobs) do
        if not session.completed then
            session.completed = true
            ExecuteSqlUpdate(
                "UPDATE noir_truckjob_deliveries SET status = 'failed_system', result_reason = 'resource_stop', finished_at = NOW() WHERE session_id = ? AND status = 'in_progress'",
                { session.sessionId }
            )
            ExecuteSqlUpdate(
                "UPDATE noir_truckjob_offers SET status = 'failed_system', finished_at = NOW(), result_reason = 'resource_stop' WHERE offer_id = ? AND status = 'in_progress'",
                { session.offerId }
            )
        end
    end
...
function Contracts.ReconcileAfterStart()
    ExecuteSqlUpdate("UPDATE noir_truckjob_deliveries SET status = 'failed_system', result_reason = 'resource_restart', finished_at = NOW() WHERE status = 'in_progress'")
    ExecuteSqlUpdate("UPDATE noir_truckjob_offers SET status = 'failed_system', finished_at = NOW(), result_reason = 'resource_restart' WHERE status = 'in_progress'")
end
