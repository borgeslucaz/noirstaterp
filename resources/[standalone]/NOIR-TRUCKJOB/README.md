...
Noir Truckjob is a premium open-source FiveM trucking resource with persistent driver progression, company trust, daily missions, leaderboards, legal freight, optional illegal cargo, and a React-based NUI dispatch tablet.

![Noir Truckjob UI preview](docs/ui-preview.png)

## Noir Truck V2 — Mercado Global (Noir State)
...
- 3–7 cargas únicas por rotação (1–2 baixas, 1–2 médias, 1–3 altas), geradas de forma determinística e persistidas em `noir_truckjob_offers`.
...
- As 16 missões, 37 rotas, coordenadas, spawns, modelos e fluxos especiais do catálogo (`Config.Routes`) permanecem intactos; as ofertas apontam para `routeId` via `Config.Routes` (`shared/contract_config.lua`).
...
- Histórico normalizado em `noir_truckjob_deliveries`; migração automática e idempotente no start (`install/install.sql` documenta o esquema).
...
### Manual Setup
1. Place this folder in your server resources as `noir-truckjob`.
