...
--- @param source number
--- @return string|nil
--- @description Returns the identifier (citizenid) for the given source, or nil when unavailable.
--- @usage local identifier = ps.getIdentifier(source)
function ps.getIdentifier(source)
    local player = ps.getPlayer(tonumber(source))
    -- qbx_core removes the player object before some resources receive
    -- playerDropped. In that window GetPlayer legitimately returns nil, so
    -- consumers must be allowed to treat the identifier as unavailable.
