...
GetPlayerCharactersArray = function()
    ...
    local citizenIds = {}
    for _, slot in ipairs(option) do
        if not slot.emptyslot then citizenIds[#citizenIds + 1] = slot.citizenid end
    end

    if #citizenIds > 0 then
        local gangs = lib.callback.await('noir_multichar:server:getCharacterGangs', false, citizenIds) or {}
        for _, slot in ipairs(option) do
            local gang = gangs[slot.citizenid]
            if gang then
                slot.additionalInfo.gang = gang.label
                slot.additionalInfo.gangGrade = gang.grade
            end
        end
    end
