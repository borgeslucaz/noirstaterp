...
const Option = ({ name, handleChange, optionsPopup, handleOptionsPopup, closeOptions }) => {
  ...
  const { anchorRef, popoverRef, placement } = usePopoverPlacement(optionsPopup, closeOptions, { below: true })
  ...
  return (
    <div ref={anchorRef} className={`noir-create__field noir-create__tile noir-create__popover-anchor${optionsPopup ? ' is-open' : ''}`}>
      <span className='noir-create__label'>NACIONALIDADE</span>
      <button type='button' onClick={handleOptionsPopup} onMouseEnter={() => nuicallback('hover')} className='noir-create__control noir-create__select' aria-expanded={optionsPopup}>
        <span className={selectedCountry ? '' : 'noir-create__placeholder'}>{selectedCountry || 'SELECIONE'}</span>
      ...
      <div ref={popoverRef} className={`noir-create__popover noir-create__countries noir-create__popover--${placement}${optionsPopup ? ' noir-create__popover--open' : ''}`} aria-hidden={!optionsPopup}>
        <input className='noir-create__search' type='text' placeholder='BUSCAR PAÍS' value={searchCountry} onChange={(event) => setSearchCountry(event.target.value)} />
